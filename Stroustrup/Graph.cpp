...
namespace Graph_lib {
	...
	void Box::draw_lines() const
	{
		...
		if (color().visibility())
		{
			fl_line(point(0).x+10, point(0).y, point(0).x+maxw-10, point(0).y);
			fl_line(point(0).x+maxw, point(0).y+10, point(0).x+maxw, point(0).y+maxh-10);
			...
			fl_arc(point(0).x, point(0).y, 20, 20, 90, 180); //ëåâûé âåðõíèé óãîë
			fl_arc(point(0).x + maxw-20, point(0).y, 20, 20, 0, 90); //ïðàâûé âåðõíèé 
			fl_arc(point(0).x + maxw-20 , point(0).y + maxh-20, 20, 20, 270, 0); //íèæíèé ïðàâûé
			fl_arc(point(0).x, point(0).y+maxh-20, 20, 20, 180, 270); //íèæíèé ëåâûé
	...
	void Arc::draw_lines() const
	{
		...
		if (fill_color().visibility()) {	// fill
			fl_color(fill_color().as_int());
			fl_pie(point(0).x, point(0).y, maxw + maxw - 1, maxh + maxh - 1, start, end);
			fl_color(color().as_int());	// reset color
		}

		if (color().visibility()) {
			fl_color(color().as_int());
			fl_arc(point(0).x, point(0).y, maxw + maxw, maxh + maxh, start, end);
		}
