...
#undef minor
	...
	struct Arc : Shape {
		...
		Arc(Point xy, int rw, int rh, int st, int finish) : 
			maxw(rw), maxh(rh), start(st), end(finish)	{
			Point lu;
			lu.x = xy.x - maxw;
			lu.y = xy.y - maxh;		//ââîäèì òî÷êó îòêóäà áóäåò âûõîäèòü äóãà, à â ïÿìÿòü ïîéäåò ëåâûé âåðõíèé óãîë îïèñàííîãî êâàäðàòà
			add(lu);
		}

		void draw_lines() const;

		// ìîæíî äîáàâèòü ãåòòåðû
	...
	struct Box : Shape {
		...
		Box(Point xy, int w, int h) :		//ëåâûé âåðõíèé óãîë îïèñàííîãî êâàäðàòà, äëèííà è âûñîòà, Ñîîáùåíèå.
			maxw(w), maxh(h)
		{
			add(xy);
		}
